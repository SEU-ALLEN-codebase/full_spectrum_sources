...
def load_feature(feat_file):
    ...
    tmp = df[__FEAT_NAMES__]
    df.loc[:, __FEAT_NAMES__] = (tmp - tmp.mean()) / (tmp.std() + 1e-10)
    return df
...
def calc_regional_features(df, out_dir):
    ...
    df_st = df[df.Manually_corrected_soma_region.isin(stypes)][['Manually_corrected_soma_region', *__FEAT_NAMES__]]
    ...
    mean_st.to_csv(os.path.join(out_dir, 'stype_mean_features_fullMorpho.csv'))
    std_st.to_csv(os.path.join(out_dir, 'stype_std_features_fullMorpho.csv'))

    # p-type regional features
    ptypes = PstypesToShow['CTX'] + PstypesToShow['TH'] + PstypesToShow['STR']
    df_pt = df[df.ptype.isin(ptypes)][['ptype', *__FEAT_NAMES__]]
    mean_pt = df_pt.groupby('ptype').mean().reindex(ptypes)
    std_pt = df_pt.groupby('ptype').std().reindex(ptypes)
    mean_pt.to_csv(os.path.join(out_dir, 'ptype_mean_features_fullMorpho.csv'))
    std_pt.to_csv(os.path.join(out_dir, 'ptype_std_features_fullMorpho.csv'))

    # cortical layer for CTX
    cstypes = CorticalLayers
    df_cs = df[df.cstype.isin(cstypes)][['cstype', *__FEAT_NAMES__]]
    mean_cs = df_cs.groupby('cstype').mean().reindex(cstypes)
    std_cs = df_cs.groupby('cstype').std().reindex(cstypes)
    mean_cs.to_csv(os.path.join(out_dir, 'cstype_mean_features_fullMorpho.csv'))
    std_cs.to_csv(os.path.join(out_dir, 'cstype_std_features_fullMorpho.csv'))


if __name__ == '__main__':
    feat_file = '../data/feature.txt'
    celltype_file = '../../common_lib/41586_2021_3941_MOESM4_ESM.csv'
    out_dir = '../../sd_features/data'
